...
def prioritize_high_risk_segments(model_results, telemetry_data, top_n=20):
    ...
    test_segments["criticality_score"] = (
        test_segments["pressure_bar"] * 0.6 + test_segments["flow_ksm3h"] * 10.0
    )

    test_segments["risk_rank_score"] = test_segments["risk_probability"] * (
        1 + 0.003 * test_segments["criticality_score"]
    )

    test_segments["thermal_margin"] = test_segments["wat_celsius"] - 0.5 * (
        test_segments["temp_in_celsius"] + test_segments["temp_out_celsius"]
    )
    ...
    top_segments = test_segments.sort_values("risk_rank_score", ascending=False).head(
        top_n
    )
...
def main():
    ...
    logger.info(
        f"   ✓ Risk categories: {priority_segments['risk_category'].value_counts().to_dict()}"
    )
    logger.info(
        f"   ✓ Top risk score: {priority_segments['risk_rank_score'].iloc[0]:.3f}"
    )
    ...
    categorical_cols = [
        "crude_type_light_sweet",
        "crude_type_medium",
    ]  # heavy_waxy is dropped
    ...
    logger.info(
        f"   ✓ Top 3 explain: {feature_importance.head(3)['cumulative_importance'].iloc[-1]:.0%}"
    )
