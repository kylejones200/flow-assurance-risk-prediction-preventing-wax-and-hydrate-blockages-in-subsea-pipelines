...
def generate_pipeline_telemetry(n_segments=4000, seed=77):
    ...
    crude = rng.choice(['light_sweet', 'medium', 'heavy_waxy'], n_segments, p=[0.5, 0.35, 0.15])
    wat_celsius = np.where(crude == 'heavy_waxy', rng.normal(35, 2, n_segments), rng.normal(27, 2, n_segments))
    ...
    base_logit = (
        0.8 * (thermal_margin > 0) + 0.4 * (thermal_margin > 2) +
        0.3 * (water_cut > 0.2) + 0.3 * (crude == 'heavy_waxy') -
        0.4 * inhibitor_active - 0.3 * (shear_proxy > 0.9)
    )
    ...
    return pd.DataFrame({
        'crude_type': crude, 'wat_celsius': wat_celsius,
        'temp_in_celsius': temp_in_celsius, 'temp_out_celsius': temp_out_celsius,
        'pressure_bar': pressure_bar, 'flow_ksm3h': flow_ksm3h,
        'shear_proxy': shear_proxy, 'inhibitor_active': inhibitor_active,
        'water_cut': water_cut, 'risk_observed': risk_observed
    })
...
def train_simple_model(data):
    ...
    preprocessor = ColumnTransformer([
        ...
        ('categorical', OneHotEncoder(drop='first', sparse_output=False), ['crude_type'])
    ])
    ...
    model = Pipeline([
        ...
        ('classifier', RandomForestClassifier(n_estimators=400, random_state=3, n_jobs=-1))
    ])
...
def create_main_visualization():
    ...
    thermal_margin = X_test['wat_celsius'] - 0.5 * (X_test['temp_in_celsius'] + X_test['temp_out_celsius'])
    
    # Create figure with three panels
    fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(10, 10))
    
    # Panel 1: Risk vs Thermal Margin
    ax1.scatter(thermal_margin, y_proba, s=20, alpha=0.5,
                color='white', edgecolors='black', linewidths=0.5)
    
    # Add risk threshold line
    ax1.axhline(y=0.5, color='gray', linestyle='--', linewidth=1, label='50% Risk Threshold')
    
    # Add zero margin line
    ax1.axvline(x=0, color='gray', linestyle=':', linewidth=1, label='Zero Thermal Margin')
    ...
    ax2.scatter(X_test['flow_ksm3h'], y_proba, s=20, alpha=0.5,
                color='white', edgecolors='black', linewidths=0.5)
    
    # Add risk threshold
    ax2.axhline(y=0.5, color='gray', linestyle='--', linewidth=1, label='50% Risk Threshold')
    ...
    ax3.scatter(X_test['water_cut'], y_proba, s=20, alpha=0.5,
                color='white', edgecolors='black', linewidths=0.5)
    ...
    ax3.axvline(x=0.2, color='gray', linestyle=':', linewidth=1, label='High Water Cut')
    ...
    print("✓ Created: 09_flow_assurance_main.png")
...
def create_accuracy_visualization():
    ...
    ax1.plot(fpr, tpr, color='black', linewidth=2, label=f'ROC Curve (AUC = {roc_auc:.3f})')
    ax1.plot([0, 1], [0, 1], 'k--', linewidth=1, label='Random Classifier')
    
    # Add optimal threshold point
    optimal_idx = np.argmax(tpr - fpr)
    optimal_threshold = thresholds[optimal_idx]
    ax1.scatter([fpr[optimal_idx]], [tpr[optimal_idx]], s=100,
                color='white', edgecolors='black', linewidths=2, zorder=5,
    ...
    ax2.hist(safe_segments, bins=bins, alpha=0.5, color='white',
             edgecolor='black', linewidth=1.5, label='Safe Segments')
    ax2.hist(risk_events, bins=bins, alpha=0.5, color='gray',
             edgecolor='black', linewidth=1.5, label='Risk Events')
    ...
    print("✓ Created: 09_flow_assurance_accuracy.png")

def main():
    """Generate all visualizations."""
    set_tufte_defaults()
    print("=" * 60)
    print("FLOW ASSURANCE RISK PREDICTION - VISUALIZATION GENERATION")
    print("=" * 60)
    print()
